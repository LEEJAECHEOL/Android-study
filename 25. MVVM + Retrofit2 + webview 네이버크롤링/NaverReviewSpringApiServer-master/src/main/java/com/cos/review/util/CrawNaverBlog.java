	...
	public List<Product> startDayCraw(String keyword){
		...
		while(products.size() < 100) {
			...
			try {
				...
				Elements els1 = doc.select(".total_wrap.api_ani_send .api_txt_lines.total_tit");
				Elements els2 = doc.select(".total_wrap.api_ani_send .total_sub .etc_dsc .etc_dsc_group .etc_dsc_area .sub_time");
				Elements els3 = doc.select(".total_wrap.api_ani_send .thumb_single .thumb_fix .thumb");
	...
	public List<Product> startAllCraw(String keyword){
		...
		while(products.size() < 101) {
			...
			try {
				...
				for (int i=0; i<els1.size(); i++) {
					Product product = new Product();
					product.setTitle(els1.get(i).text());
					product.setBlogUrl(els1.get(i).attr("href"));
					product.setDay(els2.get(i).text());
					if(els3.size() <= i) {
						product.setThumnail("사진없음");
